...
export function CommandPalette() {
  ...
  const actions: Action[] = useMemo(() => {
    const base: Action[] = [
      { id: "go-dashboard", section: "Navigate", label: "Dashboard", sub: "Morning glance", run: () => router.push("/dashboard") },
      ...
      { id: "go-shopping", section: "Navigate", label: "Shopping list", run: () => router.push("/shopping") },
      { id: "go-settings", section: "Navigate", label: "Settings", run: () => router.push("/settings") },
      { id: "act-add", section: "Actions", label: "Add new item", sub: "Create from scratch", run: () => router.push("/items/new") },
    ];
    ...
    <Modal open={open} onClose={() => setOpen(false)} width={620}>
      <div className="-mt-2">
        ...
        <div ref={listRef} className="max-h-105 overflow-y-auto">
          {filtered.length === 0 && (
            <div className="p-8 text-center text-ink-3 font-display italic">
              Nothing matches.
            </div>
          )}
          {grouped.map(([section, rows]) => (
            <div key={section} className="mb-3">
              <div className="caption px-3 py-1 mb-1">{section}</div>
              {rows.map(({ idx, action }) => (
                <button
                  key={action.id}
                  type="button"
                  ...
                  className="w-full text-left px-3 py-2.5 rounded-sm bg-transparent border-0 font-sans text-sm text-ink-1 cursor-pointer transition-[background] duration-120 ease-pantry flex items-center gap-2.5 hover:bg-paper-2 data-[active=true]:bg-paper-2"
                >
        ...
        <div className="flex justify-between items-center border-t border-paper-3 pt-3 mt-2">
          <span className="caption">↑ ↓ NAVIGATE · ↵ OPEN · ESC CLOSE</span>
          <span className="caption">{filtered.length} RESULT{filtered.length === 1 ? "" : "S"}</span>
        </div>
      </div>
    </Modal>
  );
