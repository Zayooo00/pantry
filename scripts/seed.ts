...
async function seed() {
  ...
  const demoUsers = [
    ...
    {
      email: "maya@pantry.local",
      name: "Maya Hsu",
      password: "password123",
    },
    {
      email: "nora@pantry.local",
      name: "Nora Park",
      password: "password123",
    },
  ];
  ...
  const roomData = [
    { id: "pantry", ownerId: alexId, name: "Pantry", glyph: "pantry", subtitle: "Main staples — dry goods, oils, condiments.", tinted: false, position: 0, archivedAt: null },
    { id: "basement", ownerId: alexId, name: "Basement", glyph: "basement", subtitle: "Long-term storage — preserves, paper, wine.", tinted: true, position: 1, archivedAt: null },
    { id: "kitchen", ownerId: alexId, name: "Kitchen", glyph: "kitchen", subtitle: "Daily reach — coffee, salt, bread.", tinted: false, position: 2, archivedAt: null },
    { id: "fridge", ownerId: alexId, name: "Fridge", glyph: "fridge", subtitle: "Cold — dairy, produce, leftovers.", tinted: false, position: 3, archivedAt: null },
  ...
  const itemData = [
    ...
    { roomId: "pantry", name: "Anchovies in oil", category: "Canned", unit: "tins", count: 5, threshold: 2, shelf: "B-06", expiresAt: day(8) },
    ...
    // Fridge — multiple expiring items
    ...
    // Maya's pantry — shared with Alex
    { roomId: "maya-pantry", name: "Plum jam, home-made", category: "Preserves", unit: "jars", count: 6, threshold: 2, shelf: "Top" },
    { roomId: "maya-pantry", name: "Sourdough crackers", category: "Baking", unit: "pkts", count: 2, threshold: 2 },
  ];
  ...
  // --- Activity history (rich event mix on the hero item) -------------------
  ...
  if (oliveOil) {
    await db.insert(itemEvents).values([
      ...
      { id: randomUUID(), itemId: oliveOil.id, userId: mayaId, kind: "consume", delta: -0.05, countAfter: 0.4, actor: "Maya", createdAt: day(-3) },
      { id: randomUUID(), itemId: oliveOil.id, userId: alexId, kind: "opened", countAfter: 1.0, actor: "Alex", createdAt: day(-10) },
      { id: randomUUID(), itemId: oliveOil.id, userId: alexId, kind: "restock", delta: 1.0, countAfter: 1.0, actor: "Alex", note: "Olive Tree Grocers", createdAt: day(-12) },
    ]);
  }
  const flour = enriched.find((i) => i.name.includes("00 pizza flour"));
  if (flour) {
    await db.insert(itemEvents).values([
      { id: randomUUID(), itemId: flour.id, userId: null, kind: "low_threshold_crossed", countAfter: 0.5, actor: "auto", createdAt: day(-2) },
      { id: randomUUID(), itemId: flour.id, userId: alexId, kind: "consume", delta: -0.5, countAfter: 0.5, actor: "Alex", createdAt: day(-2) },
    ]);
  }

  // --- Shopping list (auto-built + manual + completed) ----------------------
  ...
  // Maya also gets one notification (since she's a member of Pantry where flour dropped).
  if (flour) {
    await db.insert(notifications).values({
      id: randomUUID(),
      userId: mayaId,
  ...
  console.log(
    `Seeded ${roomData.length} rooms (1 archived, 1 shared in), ${enriched.length} items, ` +
      `${demoUsers.length} users, 1 pending invite, ${oliveOil ? 3 : 2} notifications for Alex.`,
  );
  process.exit(0);
}
