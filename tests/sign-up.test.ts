...
describe("POST /api/sign-up", () => {
  ...
  it("rejects when email is invalid", async () => {
    const res = await POST(jsonReq({ name: "Alex", email: "not-an-email", password: "hunter2hunter" }));
    expect(res.status).toBe(400);
  });

  it("rejects when name is empty", async () => {
    const res = await POST(jsonReq({ name: "", email: "alex@example.com", password: "hunter2hunter" }));
    expect(res.status).toBe(400);
  });
  ...
  it("returns 409 when email already exists (case-insensitive)", async () => {
    ...
    const res = await POST(jsonReq({ name: "Other", email: "ALEX@example.com", password: "different8" }));
  ...
  it("reproduces the user's failing payload (test@test.pl with 12-char password)", async () => {
    const res = await POST(jsonReq({ name: "test@test.pl", email: "test@test.pl", password: "test@test.pl" }));
    expect(res.status).toBe(200);
  });
});
